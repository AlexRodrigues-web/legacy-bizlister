<?php
/****************************************************
 * header_subcategory.php  (COM ERROR LOG + HELPERS)
 * - Normaliza $SiteLink (evita http:// duplicado)
 * - Inclui helpers de imagem (placeholder/thumb)
 * - MantÃ©m navbar/busca originais
 ****************************************************/
...
error_log("===== [header_subcategory.php] request: " . ($_SERVER['REQUEST_URI'] ?? '') . " =====");
...
/* Normaliza $SiteLink UMA VEZ (sem duplicar http://) */
...
/* =============== HELPERS DE IMAGEM (compartilhÃ¡veis) =============== */
function normalize_http_dup(string $url): string {
    // remove http(s):// duplicado no inÃ­cio
    return preg_replace('~^(https?://)+~i', '$1', $url);
}
/** Placeholder como caminho RELATIVO para evitar problemas com pasta com espaÃ§o */
...
function biz_original_image_url(?string $featured): ?string {
    ...
    // Se jÃ¡ Ã© URL absoluta, normaliza e retorna
    if (preg_match('~^https?://~i', $featured)) {
        return normalize_http_dup($featured);
    }
    // Retorna caminho relativo para uploads (evita incluir pasta com espaÃ§o)
    return 'uploads/' . rawurlencode($featured);
}
/** Monta src para thumbs.php, com fallback seguro em placeholder */
function biz_thumb_src(?string $featured, int $h=300, int $w=500, int $q=100, ?int $bizId=null): string {
    ...
    // Se vazio ou "uploads/" sem arquivo -> placeholder
    if (!$orig || preg_match('~/(uploads/?|uploads)$~i', $orig)) {
        $ph = home_placeholder_url(); // relativo
        $who = $bizId !== null ? "biz_id={$bizId}" : "sem biz_id";
        error_log("[placeholder][header_subcategory] usando placeholder ({$who}); src={$ph}");
        return 'thumbs.php?src=' . rawurlencode($ph) . "&h={$h}&w={$w}&q={$q}";
    }

    // Garante que espaÃ§os virem %20
    $orig = preg_replace('/\s+/', '%20', normalize_http_dup($orig));
...
/* =============== SUBCATEGORIA: captura segura de $id + CatRow =============== */
$id = 0;
if (isset($_GET['id']) && $_GET['id'] !== '') {
    $id = (int) $_GET['id'];
}
if ($id === 0 && !empty($_SERVER['REQUEST_URI'])) {
    if (preg_match('~subcategory-(\d+)-~i', $_SERVER['REQUEST_URI'], $m)) {
        $id = (int) $m[1];
        error_log("[header_subcategory] ID extraÃ­do da URL bonita: {$id}");
    }
}
...
if ($id > 0) {
    if ($CatSql = @$mysqli->query("SELECT * FROM categories WHERE cat_id='{$id}' LIMIT 1")) {
        if ($tmp = $CatSql->fetch_assoc()) {
            $CatRow = $tmp;
            $CatId  = (int)$CatRow['cat_id'];
            $CName1 = (string)$CatRow['category'];
            $CDesc1 = (string)($CatRow['cat_description'] ?? '');
            $CLink1 = strtolower(urlencode(preg_replace("![^a-z0-9]+!i", "-", $CName1)));
        }
        $CatSql->close();
    }
}
...
<head>
<meta charset="utf-8">
<title><?php echo htmlspecialchars($CName1, ENT_QUOTES, 'UTF-8'); ?> | <?php echo htmlspecialchars($SiteTitle, ENT_QUOTES, 'UTF-8'); ?></title>
<meta name="description" content="<?php echo htmlspecialchars($CDesc1, ENT_QUOTES, 'UTF-8'); ?>" />
...
<meta property="og:url"         content="<?php echo htmlspecialchars($SiteLink . '/subcategory-' . $CatId . '-' . $CLink1, ENT_QUOTES, 'UTF-8'); ?>" />
<meta property="og:title"       content="<?php echo htmlspecialchars($CName1, ENT_QUOTES, 'UTF-8'); ?>" />
<meta property="og:description" content="<?php echo htmlspecialchars($CDesc1, ENT_QUOTES, 'UTF-8'); ?>" />
...
$(function(){ $.fn.raty.defaults.path = 'templates/<?php echo htmlspecialchars($Template, ENT_QUOTES, 'UTF-8'); ?>/images'; new WOW().init(); });
</script>
</head>
...
<nav class="navbar navbar-default navbar-fixed-top" role="navigation">
  ...
  <div class="container">
    <div class="navbar-header">
      ...
      <!-- NÃƒO prefixe http:// manualmente; $SiteLink jÃ¡ estÃ¡ normalizado -->
